    ...
    breakdown:
      params.breakdown === "category"
        ? categoryBreakdown.map((cat) => ({
            category: cat.name,
            amount: Number.parseFloat(cat.amount.toFixed(2)),
...
export function generateCreatedInvoice(params: any) {
  return {
    ...
    message: `Invoice ${
      params.sendImmediately ? "created and sent" : "created as draft"
    }`,
  };
}
...
export function generateDataExport(params: any) {
  return {
    ...
    period: { from: params.from, to: params.to },
    downloadUrl: `https://example.com/exports/${faker.string.uuid()}.${
      params.format
    }`,
